...
# 13 Head top       added
# ----------------------------------------------------
def gener_matrix(j2d1, j2d2):
    smpl_ids = [8, 5, 2, 1, 4, 7, 21, 19, 17, 16, 18, 20]
    A = np.eye(72)
    dif = (j2d1-j2d2)**2
    norm = dif[:,0] + dif[:,1]
    for ind, ids in enumerate(smpl_ids):
        A[ids, ids] = 1/(norm[ind]+1e-12)
    return A
...
def optimize_on_joints(j2d,
    ...
    for o_id, orient in enumerate(orientations):
        ...
        alpha = 10
        # Modif
        if(A is not None and old_pose is not None):
...
def run_single_fit(img,
    ...
    return params, images, cam, try_both_orient, body_orient, sv.betas, sv.pose
...
def main(base_dir,
    ...
    for ind, img_path in enumerate(img_paths):
        ...
        if not exists(out_path):
            ...
            params, vis, cam, try_both_orient, body_orient = run_single_fit(
                img,
                joints,
                conf,
                model,
