...
ADDED : compute it according to the previous frame and change initialization
    """
    t0 = time()
    ...
    for o_id, orient in enumerate(orientations):
        ...
        if(betas_init is None):
            ...
            betas = ch.array(betas_init.copy())
        ...
        if(betas_init is None):
            ...
            opt_weights = zip([4.04 * 1e1, 4.04 * 1e1, 5.74, 0.478],
                              [1e2, 5 * 1e1, 1e1, .5 * 1e1])
...
def run_single_fit(img,
    ...
    return params, images, cam, try_both_orient, body_orient
